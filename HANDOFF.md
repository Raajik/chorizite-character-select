...
- Reading user screenshots: **OCR + pixel-map tool built and working**: `A:\tmp\ocrprobe` (`dotnet run -- <image.png>`). It's a net8.0-windows10.0.19041.0 console app (WinRT projections work there, unlike PowerShell): prints a 96-col ASCII brightness map of the image (layout geometry) + Windows OCR text lines with word positions. Rebuild with `dotnet run` in that dir. Limitation remains: no semantic vision (can't see art/colors directly — brightness map + OCR only).
